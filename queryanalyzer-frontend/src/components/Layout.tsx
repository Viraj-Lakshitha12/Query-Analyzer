...
import { Activity, Database, AlertTriangle, Settings, LogOut, Code2, Sun, Moon, Laptop, Menu, ChevronRight, Server, PieChart, X } from 'lucide-react';
...
export default function Layout() {
  ...
  if (authLoading || appLoading) return <div className="min-h-screen flex items-center justify-center text-foreground">Loading...</div>;
  ...
  const SidebarContent = () => (
    <>
      ...
      <div className="h-16 flex items-center px-6 border-b border-border flex-shrink-0">
        ...
        <span className="font-bold text-lg tracking-tight ml-2 truncate">QueryLens</span>
      </div>

      {/* App Selector */}
      <div className="p-4 border-b border-border flex-shrink-0">
        <label className="text-xs font-medium text-muted-foreground mb-2 block uppercase tracking-wider">Environment</label>
        <select
        ...
        {navigation.map((item) => {
          ...
          return (
            ...
            >
              ...
              <item.icon className={`w-5 h-5 flex-shrink-0 mr-3 ${isActive ? 'text-blue-500' : 'text-muted-foreground group-hover:text-foreground/80'}`} />
      ...
      <div className="p-4 border-t border-border flex items-center justify-between flex-shrink-0">
        <div className="flex items-center min-w-0 cursor-pointer group">
          ...
          <div className="ml-3 min-w-0">
            <p className="text-sm font-medium text-foreground truncate group-hover:text-blue-500 transition-colors duration-200">{user.fullName}</p>
          </div>
        </div>
        <button onClick={() => setShowLogoutModal(true)} className="text-muted-foreground hover:text-rose-500 transition-colors duration-200 p-1.5 rounded-md hover:bg-rose-500/10 flex-shrink-0 ml-2 cursor-pointer">
          ...
          <div className="bg-card border border-border rounded-lg shadow-xl w-full max-w-sm overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            <div className="p-6">
              ...
              <p className="text-sm text-muted-foreground mb-6">
                Are you sure you want to log out of QueryLens? You will need to sign in again to access your environments.
              </p>
